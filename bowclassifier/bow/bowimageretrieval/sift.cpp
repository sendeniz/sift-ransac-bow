...
SIFT_SALIENT_METHOD::CommonParams::CommonParams() :
        nOctaves(DEFAULT_NOCTAVES), nOctaveLayers(DEFAULT_NOCTAVE_LAYERS),
        firstOctave(DEFAULT_FIRST_OCTAVE), angleMode(FIRST_ANGLE)
{}

SIFT_SALIENT_METHOD::CommonParams::CommonParams( int _nOctaves, int _nOctaveLayers, int /*_firstOctave*/, int /*_angleMode*/ ) :
        nOctaves(_nOctaves), nOctaveLayers(_nOctaveLayers),
        firstOctave(-1/*_firstOctave*/), angleMode(FIRST_ANGLE/*_angleMode*/)
{}

SIFT_SALIENT_METHOD::CommonParams::CommonParams( int _nOctaves, int _nOctaveLayers ) :
        nOctaves(_nOctaves), nOctaveLayers(_nOctaveLayers), firstOctave(-1), angleMode(FIRST_ANGLE)
{}

SIFT_SALIENT_METHOD::DetectorParams::DetectorParams() :
        threshold(GET_DEFAULT_THRESHOLD()), edgeThreshold(GET_DEFAULT_EDGE_THRESHOLD())
{}

SIFT_SALIENT_METHOD::DetectorParams::DetectorParams( double _threshold, double _edgeThreshold ) :
        threshold(_threshold), edgeThreshold(_edgeThreshold)
{}

SIFT_SALIENT_METHOD::DescriptorParams::DescriptorParams() :
        magnification(GET_DEFAULT_MAGNIFICATION()), isNormalize(DEFAULT_IS_NORMALIZE),
        recalculateAngles(true)
{}

SIFT_SALIENT_METHOD::DescriptorParams::DescriptorParams( double _magnification, bool /*_isNormalize*/, bool _recalculateAngles ) :
        magnification(_magnification), isNormalize(true/*_isNormalize*/),
        recalculateAngles(_recalculateAngles)
{}

SIFT_SALIENT_METHOD::DescriptorParams::DescriptorParams( bool _recalculateAngles )
    : magnification(GET_DEFAULT_MAGNIFICATION()), isNormalize(true),recalculateAngles(_recalculateAngles)
{}

SIFT_SALIENT_METHOD::SIFT_SALIENT_METHOD()
{}

SIFT_SALIENT_METHOD::SIFT_SALIENT_METHOD( double _threshold, double _edgeThreshold, int _nOctaves,
...
SIFT_SALIENT_METHOD::SIFT_SALIENT_METHOD( double _magnification, bool _isNormalize, bool _recalculateAngles, int _nOctaves,
            int _nOctaveLayers, int _firstOctave, int _angleMode )
{
...
SIFT_SALIENT_METHOD::SIFT_SALIENT_METHOD( const CommonParams& _commParams,
...
int SIFT_SALIENT_METHOD::descriptorSize() const
{
    return DescriptorParams::DESCRIPTOR_SIZE;
}

SIFT_SALIENT_METHOD::CommonParams SIFT_SALIENT_METHOD::getCommonParams () const
{
    return commParams;
}

SIFT_SALIENT_METHOD::DetectorParams SIFT_SALIENT_METHOD::getDetectorParams () const
{
    return detectorParams;
}

SIFT_SALIENT_METHOD::DescriptorParams SIFT_SALIENT_METHOD::getDescriptorParams () const
{
...
inline cv::KeyPoint featureToKeyPoint( const feature& feat )
{
    float size = (float)(feat.scl * SIFT_SALIENT_METHOD::DescriptorParams::GET_DEFAULT_MAGNIFICATION() * 4); // 4==NBP
...
inline void keyPointToFeature( const cv::KeyPoint& keypoint, feature& feat, const SiftParams& params )
{
    feat.x = keypoint.pt.x;
    feat.y = keypoint.pt.y;

    feat.scl = keypoint.size / (SIFT_SALIENT_METHOD::DescriptorParams::GET_DEFAULT_MAGNIFICATION()*4); // 4==NBP
...
// detectors
void SIFT_SALIENT_METHOD::operator()(const cv::Mat& image, const cv::Mat& mask,
...
// duplicated twice.
void recalculate_Angles( std::vector<cv::KeyPoint>& keypoints, IplImage*** gauss_pyr,
                      ...
                      bool useProvidedKeypoints) const
{
    ...
    if( descriptorParams.recalculateAngles )
        recalculate_Angles( keypoints, pyrImages.gauss_pyr, commParams.nOctaves, commParams.nOctaveLayers );
    ...
    descriptors.create( featuresSeq->total, SIFT_SALIENT_METHOD::DescriptorParams::DESCRIPTOR_SIZE, CV_32FC1 );
    for( int i = 0; i < featuresSeq->total; i++ )
    {
 ...
 *   SiftFeatureDetector
 */
Sift_FeatureDetector::Sift_FeatureDetector( const SIFT_SALIENT_METHOD::DetectorParams &detectorParams,
                                          const SIFT_SALIENT_METHOD::CommonParams &commonParams )
...
Sift_FeatureDetector::Sift_FeatureDetector( double threshold, double edgeThreshold,
...
void Sift_FeatureDetector::read( const cv::FileNode& fn )
{
    ...
    sift = SIFT_SALIENT_METHOD(threshold, edgeThreshold, nOctaves, nOctaveLayers, firstOctave, angleMode);
}

void Sift_FeatureDetector::write (cv::FileStorage& fs) const
{
    //fs << "algorithm" << getAlgorithmName ();

    SIFT_SALIENT_METHOD::CommonParams commParams = sift.getCommonParams ();
    SIFT_SALIENT_METHOD::DetectorParams detectorParams = sift.getDetectorParams ();
...
void Sift_FeatureDetector::detectImpl( const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, const cv::Mat& mask ) const
{
...
\****************************************************************************************/
Sift_DescriptorExtractor::Sift_DescriptorExtractor(const SIFT_SALIENT_METHOD::DescriptorParams& descriptorParams,
												 const SIFT_SALIENT_METHOD::CommonParams& commonParams)
												 : sift( descriptorParams.magnification, descriptorParams.isNormalize, descriptorParams.recalculateAngles,
												 commonParams.nOctaves, commonParams.nOctaveLayers, commonParams.firstOctave, commonParams.angleMode )
{}

Sift_DescriptorExtractor::Sift_DescriptorExtractor( double magnification, bool isNormalize, bool recalculateAngles,
												 int nOctaves, int nOctaveLayers, int firstOctave, int angleMode )
												 : sift( magnification, isNormalize, recalculateAngles, nOctaves, nOctaveLayers, firstOctave, angleMode )
{}

void Sift_DescriptorExtractor::computeImpl( const cv::Mat& image,
										   std::vector<cv::KeyPoint>& keypoints,
										   cv::Mat& descriptors) const
{
	...
	sift(grayImage, cv::Mat(), keypoints, descriptors, useProvidedKeypoints);
}

void Sift_DescriptorExtractor::read (const cv::FileNode &fn)
{
	...
	sift = SIFT_SALIENT_METHOD( magnification, isNormalize, recalculateAngles, nOctaves, nOctaveLayers, firstOctave, angleMode );
}

void Sift_DescriptorExtractor::write (cv::FileStorage &fs) const
{
	//    fs << "algorithm" << getAlgorithmName ();

	SIFT_SALIENT_METHOD::CommonParams commParams = sift.getCommonParams ();
	SIFT_SALIENT_METHOD::DescriptorParams descriptorParams = sift.getDescriptorParams ();
...
int Sift_DescriptorExtractor::descriptorSize() const
{
	return sift.descriptorSize();
}

int Sift_DescriptorExtractor::descriptorType() const
{
	return CV_32FC1;
}
